...
/**
 * GET /api/wealth-progress?week=N
...
export async function POST(request: Request) {
  ...
  const existing = await sqlQuery<Array<{ is_sealed: boolean }>>(
    `SELECT is_sealed FROM wealth_progress
  ...
  if (seal) {
    ...
    if (weekNumber > 0) {
      const priorUnsealedRows = await sqlQuery<Array<{ cnt: string }>>(
        `SELECT COUNT(*) as cnt FROM wealth_progress
    ...
    await sqlQuery(
      `INSERT INTO wealth_progress (id, user_id, week_number, progress_data)
       ...
       WHERE wealth_progress.is_sealed = false`,
