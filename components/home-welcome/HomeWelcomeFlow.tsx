...
/**
 * Wraps the home page content. For first-time users:
 * - Mini-app: shows Farcaster connect button (uses SDK context for auth)
 * - Browser: shows wallet connect prompt
 */
export default function HomeWelcomeFlow({ children, onAuthenticated }: HomeWelcomeFlowProps) {
    ...
    (async () => {
      try {
        ...
        if (data?.user) {
          ...
          // In mini-app: show Farcaster connect button
  ...
  const handleFarcasterConnect = useCallback(async () => {
    ...
    try {
      ...
      if (!fid) {
        console.error('[HomeWelcomeFlow] No FID in SDK context');
        setIsProcessing(false);
        return;
      }
